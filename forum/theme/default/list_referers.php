...
if (__fud_real_user__ && $FUD_OPT_1 & 1024) {
	...
	if (($ttl = q_singleval('select count(distinct(referer_id)) from fud26_users WHERE referer_id>0'))) {
		...
		$c = q('SELECT u2.alias, u2.id, count(*) AS cnt FROM fud26_users u LEFT JOIN fud26_users u2 ON u2.id=u.referer_id WHERE u.referer_id > 0 AND u2.id IS NOT NULL GROUP BY u2.id ORDER BY cnt, u.alias DESC LIMIT '.qry_limit($MEMBERS_PER_PAGE, $start));
		while ($r = db_rowarr($c)) {
			$refered_entry_data = '';
...
<tr><th class="ac">Member</th><th class="ac">Referrals</th><th>Members Referred</th></tr>
