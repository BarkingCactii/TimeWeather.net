...
}function alt_var($key)
{
	...
	if (isset($_GET['unread'], $_GET['th']) && _uid) {
		$_GET['goto'] = q_singleval('SELECT m.id from fud26_msg m LEFT JOIN fud26_read r ON r.thread_id=m.thread_id AND r.user_id='._uid.' WHERE m.thread_id='.$_GET['th'].' AND m.apr=1 AND m.post_stamp>CASE WHEN (r.last_view IS NOT NULL OR r.last_view>'.$usr->last_read.') THEN r.last_view ELSE '.$usr->last_read.' END');
		if (!$_GET['goto']) {
	...
	if (!empty($_GET['goto'])) {
		if ($_GET['goto'] === 'end' && isset($_GET['th'])) {
			...
			if (!isset($_GET['th'])) {
				$_GET['th'] = (int) q_singleval('SELECT thread_id FROM fud26_msg WHERE id='.$_GET['goto']);
			}
			if (!($pos = q_singleval("SELECT count(*) FROM fud26_msg WHERE thread_id=".$_GET['th']." AND id<=".$_GET['goto']." AND apr=1"))) {
				invl_inp_err();
			}
	...
	$frm = db_sab('SELECT
		...
		FROM fud26_thread t
			...
			'.(_uid ? 'LEFT  JOIN fud26_thread_notify 	tn ON tn.user_id='._uid.' AND tn.thread_id='.$_GET['th'].'
			LEFT  JOIN fud26_mod 		mo ON mo.user_id='._uid.' AND mo.forum_id=t.forum_id
			LEFT  JOIN fud26_thread_rate_track 	tr ON tr.thread_id='.$_GET['th'].' AND tr.user_id='._uid.'
	...
	if ($frm->moved_to) { /* moved thread, we could handle it, but this case is rather rare, so it's cleaner to redirect */
		if ($FUD_OPT_2 & 32768) {
			header('Location: http://timeweather.net/forum/index.php/m/'.$frm->root_msg_id.'/'._rsidl.'#msg_'.$frm->root_msg_id);
		} else {
			header('Location: http://timeweather.net/forum/index.php?t=msg&goto='.$frm->root_msg_id.'&'._rsidl.'#msg_'.$frm->root_msg_id);
		}
		exit;
	}
	...
	if (_uid) {
		...
		$first_unread_message_link = (($total - $_GET['th']) > $count) ? '| <a href="index.php?t=msg&amp;unread=1&amp;th='.$_GET['th'].'&amp;'._rsid.'" title="Go to the 1st unread message inside this topic">Go to unread message(s)</a>&nbsp;' : '';
		$subscribe_status = $frm->subscribed ? '| <a href="index.php?t=msg&amp;th='.$_GET['th'].'&amp;notify='.$usr->id.'&amp;'._rsid.'&amp;opt=off&amp;start='.$_GET['start'].'&amp;SQ='.$GLOBALS['sq'].'" title="Stop receiving notifications about new messages in this topic">Unsubscribe from topic</a>&nbsp;' : '| <a href="index.php?t=msg&amp;th='.$_GET['th'].'&amp;notify='.$usr->id.'&amp;'._rsid.'&amp;opt=on&amp;start='.$_GET['start'].'&amp;SQ='.$GLOBALS['sq'].'" title="Receive notification about new messages inside this topic">Subscribe to topic</a>&nbsp;';
	} else {
...
</td><td>&nbsp;<input type="submit" class="button" name="btn_vote" value="Rate">
<input type="hidden" name="rate_thread_id" value="'.$frm->id.'">'._hs.'
	...
	if ($frm->replies > 250) {
		q("CREATE TEMPORARY TABLE fud26__mtmp_".__request_timestamp__." AS SELECT id FROM fud26_msg WHERE thread_id=".$_GET['th']." AND apr=1 ORDER BY id ASC LIMIT " . qry_limit($count, $_GET['start']));
	}
	...
	FROM '.($frm->replies > 250 ? 'fud26__mtmp_'.__request_timestamp__.' mt INNER JOIN fud26_msg m ON m.id=mt.id' : ' fud26_msg m').'
	...
	if ($FUD_OPT_2 & 32768) {
		$page_pager = tmpl_create_pager($_GET['start'], $count, $total, 'index.php/mv/msg/' . $_GET['th'] . '/0/', '/' . reveal_lnk . unignore_tmp . _rsid);
	} else {
		$page_pager = tmpl_create_pager($_GET['start'], $count, $total, 'index.php?t=msg&amp;th=' . $_GET['th'] . '&amp;prevloaded=1&amp;' . _rsid . reveal_lnk . unignore_tmp);
	}
...
<?php echo (($frm->replies && $perms & 2048) ? '<a href="index.php?t=split_th&amp;'._rsid.'&amp;th='.$_GET['th'].'">Split Topic</a>&nbsp;|&nbsp;' : ''); ?>
<?php echo ($perms & 4096 ? ($frm->thread_opt & 1 ? '<a href="index.php?t=mmod&amp;'._rsid.'&amp;th='.$_GET['th'].'&amp;unlock=1&amp;SQ='.$GLOBALS['sq'].'">Unlock Topic</a>&nbsp;|&nbsp;' : '<a href="index.php?t=mmod&amp;'._rsid.'&amp;th='.$_GET['th'].'&amp;lock=1&amp;SQ='.$GLOBALS['sq'].'">Lock Topic</a>&nbsp;|&nbsp;'):'').'
'.($FUD_OPT_2 & 1073741824 ? '<a href="index.php?t=remail&amp;th='.$_GET['th'].'&amp;'._rsid.'" title="Send the URL to this page to your friend(s) via e-mail">E-mail to friend</a>&nbsp;' : ''); ?>
<?php echo $first_unread_message_link.$subscribe_status; ?></td>
<td class="vb ar"><?php echo ($FUD_OPT_3 & 2 ? '' : '<a href="index.php?t=tree&amp;th='.$_GET['th'].'&amp;'._rsid.'"><img title="Switch to threaded view of this topic" alt="Switch to threaded view of this topic" src="theme/default/images/tree_view.gif" /></a>&nbsp;'); ?><a href="index.php?t=post&amp;frm_id=<?php echo $frm->forum_id; ?>&amp;<?php echo _rsid; ?>"><img alt="Create a new topic" src="theme/default/images/new_thread.gif" /></a><?php echo ((!($frm->thread_opt & 1) || $perms & 4096) ? '&nbsp;<a href="index.php?t=post&amp;th_id='.$_GET['th'].'&amp;reply_to='.$frm->root_msg_id.'&amp;'._rsid.'&amp;start='.$_GET['start'].'"><img src="theme/default/images/post_reply.gif" alt="Submit Reply" /></a>' : ''); ?></td>
</tr>
</table>
...
<td class="GenText ar vb nw"><?php echo ($FUD_OPT_3 & 2 ? '' : '<a href="index.php?t=tree&amp;th='.$_GET['th'].'&amp;'._rsid.'"><img title="Switch to threaded view of this topic" alt="Switch to threaded view of this topic" src="theme/default/images/tree_view.gif" /></a>&nbsp;'); ?><a href="index.php?t=post&amp;frm_id=<?php echo $frm->forum_id; ?>&amp;<?php echo _rsid; ?>"><img alt="Create a new topic" src="theme/default/images/new_thread.gif" /></a><?php echo ((!($frm->thread_opt & 1) || $perms & 4096) ? '&nbsp;<a href="index.php?t=post&amp;th_id='.$_GET['th'].'&amp;reply_to='.$frm->root_msg_id.'&amp;'._rsid.'&amp;start='.$_GET['start'].'"><img src="theme/default/images/post_reply.gif" alt="Submit Reply" /></a>' : ''); ?></td>
</tr>
</table>
...
<div class="ar SmallText"><?php echo ($FUD_OPT_2 & 2097152 ? '[ <a href="'.$GLOBALS['WWW_ROOT'].'pdf.php?th='.$_GET['th'].'">Generate printable PDF</a> ]' : ''); ?> <?php echo ($FUD_OPT_2 & 1048576 ? '[ <a href="index.php?t=help_index&amp;section=boardusage#syndicate">Syndicate this forum (XML)</a> ]' : ''); ?></div>
<?php echo ($msg_page_focus ? '<script language="javascript" type="text/javascript"> fud_msg_focus("'.$mid.'"); </script>' : ''); ?>
