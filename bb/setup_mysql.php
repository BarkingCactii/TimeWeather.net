...
Copyright (C) 2001-2004 miniBB.net.
...
if(!$sus) $result=mysql_query("SELECT poster_id, poster_name, post_time, post_text, poster_ip, post_status, post_id FROM $Tp WHERE topic_id=$topic ORDER BY post_id $srt $makeLim");
...
elseif($n==6){ 
if($tr1=mysql_query("SELECT count(*) FROM $Tp WHERE topic_id=$topic") and mysql_num_rows($tr1)>0) $row=mysql_result($tr1,0); 
}

elseif($n==95){
/*Get forum replies count*/
$row=mysql_result(mysql_query("select count(*) from $Tp where forum_id=$forum"),0);
}
...
elseif($n==9)
$row=mysql_result(mysql_query("SELECT count(*) FROM $Tt WHERE forum_id=$forum"),0);
...
if ($extra==1) $xtr=getClForums($closedForums,'AND',$Tp,'forum_id','AND','!='); else $xtr='';
$xtr2=getByDay('AND',$Tp,'post_time');
$row=mysql_result(mysql_query("SELECT count(*) FROM $Tp, $Tt WHERE $Tp.topic_id=$Tt.topic_id AND (( $searchString ) OR ( $searchString2 )) $xtr $xtr2 "),0);
}
...
global $searchString, $searchHow;
if ($extra==1) $xtr=getClForums($closedForums,'AND',$Tt,'forum_id','AND','!='); else $xtr='';
if($searchHow==3||$searchString=='') $and=''; else $and='AND';
$xtr2=getByDay($and,'','topic_time');
$row=mysql_result(mysql_query("SELECT count(*) FROM $Tt WHERE $searchString $xtr $xtr2 "),0);
}
...
$row=mysql_result(mysql_query("SELECT count(*) FROM $Tp WHERE topic_id=$topic AND post_id$sign$sus"),0);
}

elseif($n==76){
/*Search - this one used in search func. for reverse page redirects only(topics by time)*/
$row=mysql_result(mysql_query("SELECT count(*) FROM $Tt WHERE forum_id=$forum AND topic_id<=$sus"),0);
}
...
$row=mysql_result(mysql_query("SELECT count(*) FROM $Tt, $Tp WHERE $Tt.topic_last_post_id=$Tp.post_id AND $Tt.forum_id=$forum AND $Tt.topic_last_post_id>$sus"),0);
}
...
$row=mysql_result(mysql_query("SELECT count(*) FROM $Tu WHERE user_id>1"),0);

elseif($n==37)
/*Stats Stuff - Num Topics*/
$row=mysql_result(mysql_query("SELECT count(*) FROM $Tt"),0);

elseif($n==38)
/*Stats Stuff - Num Posts*/
$row=mysql_result(mysql_query("SELECT count(*) FROM $Tp"),0);
...
if ($extra==1) $xtr=getClForums($closedForums,'AND',$Tt,'forum_id','AND','!='); else $xtr='';
$xtr2=getByDay('AND',$Tp,'post_time');
$result=mysql_query("SELECT $Tt.topic_id, $Tt.topic_title, $Tt.forum_id, count(*) as cnt FROM $Tt, $Tp WHERE $Tt.topic_id=$Tp.topic_id $xtr $xtr2 GROUP BY $Tp.topic_id ORDER BY cnt DESC, $Tt.topic_id DESC $makeLim ");
}
if($result) $row=mysql_fetch_row($result);
}
...
if(!$sus) $result=mysql_query("SELECT $Tu.user_id, $Tu.username, count(*) as cnt FROM $Tu, $Tp WHERE $Tu.user_id>1 AND $Tu.user_id=$Tp.poster_id GROUP BY $Tp.poster_id ORDER BY cnt DESC, $Tu.user_id DESC $makeLim ");
if($result) $row=mysql_fetch_row($result);
}
...
$result=mysql_query("SELECT $Tf.forum_id, $Tf.forum_name, count(*) as cnt FROM $Tf, $Tp WHERE $Tf.forum_id=$Tp.forum_id AND $Tp.poster_id=$userID $xtr GROUP BY $Tp.forum_id ORDER BY cnt DESC, $Tf.forum_order, forum_name");
}
...
/*Anti-Spam*/
if($postRange) $z=mysql_result(mysql_query("SELECT count(*) FROM $Tp WHERE poster_ip='$poster_ip' AND now()-post_time<$postRange"),0);
...
elseif($n==28){
/*Num forums*/
$res=mysql_query("SELECT count(*) FROM $Tf");
...
global $user;
$result = mysql_query("select count(*) from $Tp where poster_id!=0 and poster_id=$user");
if ($result) $row = mysql_fetch_row($result); else $row=FALSE;
}
...
global $user;
$result = mysql_query("select count(*) from $Tt where topic_poster!=0 and topic_poster=$user");
if ($result) $row = mysql_fetch_row($result); else $row=FALSE;
}
...
if ($extra==1) $what='count(*)'; else $what='*';
